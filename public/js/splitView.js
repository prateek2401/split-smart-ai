...
const splitView = {
  ...
  renderBalances() {
    ...
    if (totalOwed) totalOwed.textContent = `$${d.totalYouAreOwed.toFixed(2)}`;
    if (totalOwe) totalOwe.textContent = `$${d.totalYouOwe.toFixed(2)}`;
    if (dashOwed) dashOwed.textContent = `$${d.totalYouAreOwed.toFixed(2)}`;
    if (dashOwe) dashOwe.textContent = `$${d.totalYouOwe.toFixed(2)}`;

    if (netBal) {
      const isPositive = d.netBalance >= 0;
      netBal.textContent = `${isPositive ? '+' : '-'}$${Math.abs(d.netBalance).toFixed(2)}`;
      ...
      return `
        <div class="friend-card">
          ...
          <div class="friend-balance-badge">
            ...
            <div class="balance-amount" style="color: ${amountColor};">$${Math.abs(net).toFixed(2)}</div>
          </div>
        </div>
      `;
    }).join('');
    ...
    if (settleSelect && this.splitData.friends) {
      settleSelect.innerHTML = `
        ...
        ${this.splitData.friends.map(f => {
          const suffix = f.netBalance < 0 ? ` (You owe $${Math.abs(f.netBalance).toFixed(2)})` : f.netBalance > 0 ? ` (Owes you $${f.netBalance.toFixed(2)})` : ' (Settled)';
  ...
  async submitSettlement(e) {
    ...
    try {
      ...
      if (data.success) {
        app.showToast(`✅ Settled $${amount.toFixed(2)} with friend!`);
        app.closeModals();
        app.refreshAll();
      }
    } catch (err) {
